...
void MML_Synth::reset(){
  playduration = 0;
  endTick = 0;
  flgdbug = 0;
  flgRun = 0;
  common_tempo = MML_tempo;
  common_len   = MML_len;
  common_oct   = MML_oct;
  common_vol   = MML_vol;
}
...
// TONE ピッチ [,ボリューム]
// pitch  ノート番号 (NOTE_B0=23 ～ NOTE_DS8=111)
// vol    音の大きさ
void MML_Synth::tone(uint8_t pitch, uint16_t vol) {
  if (func_tone != 0)
    func_tone(idx, pitch, vol);
}

//　NOTONE
void MML_Synth::notone() {
  if (func_notone != 0)
    func_notone(idx);
}

void MML_Synth::instrument(uint8_t value) {
  if (func_instrument != 0)
    func_instrument(idx, value);
}

// 1文字出力
void  MML_Synth::debug(uint8_t c) {
  if (func_putc != 0)
    func_putc(idx,c);
}

//  演奏キューチェック
uint8_t MML_Synth::available(uint32_t tick) {
  if (!flgRun)
    return 0;
  if (playduration) {
    if ( endTick < tick ) {
        if (!flgR) notone();
        playduration = 0;
        // endTick はそのまま保持し、次の音符の基準時刻として playTick() で使う
        return 1;
    } else {
      return 0;
    }
...
// TEMPO テンポ
void MML_Synth::tempo(int16_t tempo) {
...
void MML_Synth::play(uint8_t mode) {  
...
// PLAY 文字列
void MML_Synth::playTick(uint32_t tick, uint8_t flgTick) {
  uint8_t pitch;                     // ノート番号 (NOTE_B0=23 ～ NOTE_DS8=111)
  ...
  while(*mml_ptr) {
    ...
    // 空白はスキップ    
    if (*mml_ptr == ' ' || *mml_ptr == '&' || *mml_ptr == '|' || *mml_ptr == '\t' || *mml_ptr == '\r' || *mml_ptr == '\n') {
      mml_ptr++;
      continue;
    }
    ...
    if ( ((c >= 'A') && (c <= 'G')) || (c == 'R') ) {  //**** 音階記号 A - Z,R 
      ...
      if (flgR) {
        ...
        // 音符
        pitch = 12 * (local_oct + 1) + scale;             // ノート番号の計算
        if (flgTick) {
          playduration = duration;
          endTick = (endTick > 0 ? endTick : tick) + duration;
          tone(pitch, common_vol);                        // 音の再生(時間指定なし）
          break;
        } else {
  ...
  if ((!*mml_ptr && available(tick)) || isError() ) {
    flgRun = 0;    // 演奏終了
    playMode = 0;
  }
}
