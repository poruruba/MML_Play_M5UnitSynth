...
class MML_Synth {
  private:
    ...
    void (*func_instrument)(uint8_t value) = 0;
    void (*func_tone)(uint8_t pitch, uint16_t vol) = 0;
  ...
  public:
    ...
    void init(
       void (*f1)(void), 
       void (*f2)(uint8_t pitch, uint16_t vol),
       void (*f3)(void), 
       void (*f4)(uint8_t value) = 0,
       void (*f5)(uint8_t c) = 0    ) 
